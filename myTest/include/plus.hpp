...
TEST (RatioArithmetic, plusAndZero){
	ratio::Ratio<int> r1(1,2);
	ratio::Ratio<int> r2 = ratio::Ratio<int>::zero();
	ratio::Ratio<int> result;
	result = r1+r2;
...
TEST (RatioArithmetic, plusPluralKnowedRatio) {
	ratio::Ratio<int> r1(1,2);
	ratio::Ratio<int> r2(3,4);
...
TEST (RatioArithmetic, plusEqualRandomRatio) {
	const size_t maxSize = 1000;  // max size of the tested vectors
	std::mt19937 generator(0);
	std::uniform_int_distribution<int> uniformIntDistribution(1,maxSize);
	std::uniform_real_distribution<double> uniformDistributionValue(-int(maxSize),maxSize);
...
TEST (RatioArithmetic, plusEqualRandomRatioConvert) {
	const size_t maxSize = 1000;  // max size of the tested vectors
	std::mt19937 generator(0);
	std::uniform_int_distribution<int> uniformIntDistribution(1,maxSize);
	std::uniform_real_distribution<double> uniformDistributionValue(-int(maxSize),maxSize);
